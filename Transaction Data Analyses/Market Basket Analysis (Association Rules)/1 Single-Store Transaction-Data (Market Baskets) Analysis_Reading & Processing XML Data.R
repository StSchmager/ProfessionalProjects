...
files_selected    <- as.character(dplyr::filter(FileIndex,
                                                Store == store,
                                                Date %in% date_range)[, 1]) # Select first column "FileNames" as VECTOR

## Create list with length equal to the length of files to be read, where datasets (representing information per FILE) are saved in
...
ItemAttr    <- plyr::ldply(FileList_ItemAttr, data.frame) %>% 
...
TransItems  <- plyr::ldply(FileList_TransItems, data.frame) %>% 
...
TransAttr   <- plyr::ldply(FileList_TransAttr, data.frame) %>% 
