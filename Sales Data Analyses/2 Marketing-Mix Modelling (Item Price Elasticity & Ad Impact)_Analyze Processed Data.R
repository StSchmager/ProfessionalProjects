...
data0 <- transmute(data0,
      ...
      # Remove "Open Ring" items
      filter(!grepl("OPEN RING", Product)) %>% 
      # Remove items with neg. sales
      filter(TotalWeeklyDollarSales > 0) %>% 
      filter(TotalWeeklyUnitSales   > 0)
...
setwd("/media/radmin/ExternalHD/Projects/Sales Data Analyses/1 RD/0 Other")
         LikeCodes      <- openxlsx::read.xlsx("Price List by Like Code.xlsx", sheet = 1, cols = c(1:3)) 
colnames(LikeCodes)     <- c("UPC", "LikeCode", "Product")
         LikeCodes      <- mutate(LikeCodes,
                                         UPC = suppressWarnings(as.numeric(UPC))) %>% 
                           filter(!is.na(UPC)) %>% 
                           mutate(       UPC = as.character(UPC),
                                  LikeCode   = as.character(LikeCode))

# Join like codes to each UPC in sales data
data0 <- left_join(data0,
                   select(LikeCodes, -Product), by = "UPC") %>% 
# Create additional variable that distinguishes between UPC and like code to avoid code overlap between those types of codes
      mutate(UPC2         = as.character(paste(UPC,      "(UPC)")),
             LikeCode2    = as.character(paste(LikeCode, "(LC)")),
# Create hybrid variable (dummy) that combines codes, but avoids code overlap (if no like code exist, take UPC instead in order to group/summarize later by like code if available, otherwise (not available-- NA), by UPC; Remove dummy variables)
             LikeCode_UPC = NA)
...
Dates0 <- select(data0, Week) %>% unique() %>%
          arrange(      Week) %>%
          mutate(# Convert week date from character to date data type in order to compute other date variables
                 Week      = as.Date(Week),
...
# Check if each row is represented by one unique UPC
length(unique(Products_UPC$UPC)) == nrow(Products_UPC)
...
# Check if each row is represented by one unique LikeCode-UPC
length(unique(Products_LC$LikeCode_UPC)) == nrow(Products_LC)
...
SelectedStore     <- SelectedStores[j]
...
data1 <- filter(data0,
      ...
      # Remove sub-dept. column b/c some products are grouped inconsistently in different sub-depts. although it's the same product
      select(-SubDept) %>% 
      ...
      summarize(TotalAdFeat_abs      = sum(as.numeric(as.character(factor(AdFeature,
                ...
                TotalUnitSales        =       sum(TotalWeeklyUnitSales,                                   na.rm = T),
                ...
                AvgWeeklyProfitMargin =       mean(AvgWeeklyProfitMargin,                                 na.rm = T),
      ...
      ungroup() %>% 
      mutate(# Compute (relative) dollar/unit sales in percent of total of store    
             TotalUnitSales_rel    = TotalUnitSales/  sum(TotalUnitSales),
             TotalDollarSales_rel  = TotalDollarSales/sum(TotalDollarSales),
      ...
      # Sort items by total dollar sales in descending order
      arrange(desc(TotalUnitSales)) %>% 
      # Cumulate sales and product counts for Pareto 80-20 rule
      mutate(CumUnitSales_rel            = cumsum(TotalUnitSales_rel),
             CumProductCount_UnitSales   = cumsum(ProductCount_rel)) %>%
      
      # Sort items by total dollar sales in descending order
      arrange(desc(TotalDollarSales)) %>% 
      # Cumulate sales and product counts for Pareto 80-20 rule
      mutate(CumDollarSales_rel          = cumsum(TotalDollarSales_rel),
             CumProductCount_DollarSales = cumsum(ProductCount_rel)) %>%
      
      # Add product descriptions, and like codes
      left_join(Products_UPC, by = "UPC") %>% 
      select(#Product related variables
             Dept,
             Store,
             LikeCode,
             LikeCode_UPC,
             ...
             AvgWeeklyUnitPrice,
             AvgWeeklyUnitCost,
             # Dollar Sales rel. vars.
             CumProductCount_DollarSales,
             CumDollarSales_rel,
             TotalDollarSales_rel,
             TotalDollarSales,
             AvgDailyDollarSales,
             # Unit Sales rel. vars.
             CumProductCount_UnitSales,
             CumUnitSales_rel,
             TotalUnitSales_rel,
             TotalUnitSales,
             AvgDailyUnitSales,
             ...
             AvgDailyCOGS,
             TotalProfitDollars,
             AvgDailyProfitDollars,
             AvgWeeklyProfitMargin
             )
...
SelectedProducts        <- data1$UPC[data1$CumDollarSales_rel < .8]
#SelectedProducts        <- data1$UPC[1:100]
        ProductList     <- list()
 length(ProductList)    <- length(SelectedProducts)

for (i in 1:length(SelectedProducts)) {
SelectedProduct <- SelectedProducts[i]
#SelectedProduct <- data1$UPC[data1$UPC=="7800005081"]

print(paste("UPC", SelectedProduct, "at Store", SelectedStore, "started"))
...
is.na(results$StdWeeklyUnitPriceDev_NoAd)]      <- 0
      ...
      # If product was featured in ad at least once, build models that include ad-feature variable
      if (results$TotalAdFeat_abs > 0 && length(unique(data2$AdFeature)) > 1) { 
      ...
      # If product has changed in price, models can be built and coefficients can be saved
      if (results$AvgWeeklyUnitPriceChange > 0) { 
     ...
     #results$lm1a_b_AvgDailyUnitSales_PriceElasNotAdAdj_CIhigh   <- round(summary(lm1a_Price)$coefficients[2, 1]+c(-1, 1)*qt(.975, df = lm1a_Price$df)*summary(lm1a_Price)$coefficients[2, 2], 2)[2]
      ...
      results$lm1a_b_AvgDailyUnitSales_PriceElas2NotAdAdj         <- round(summary(lm1a_Price2)$coefficients[2, 1], 2)*(results$AvgWeeklyUnitPrice/
                                                                                                                        results$lm1a_a_AvgDailyUnitSales_AvgPrice)
      ...
      if (results$AvgWeeklyUnitPriceChange_Ad > 0) {
      ...
      # Similar model with centered prices
      
      results$lm2_RSq_PriceAd2                                    <- round(summary(lm2_PriceAd2)$r.squared,          3) # !!! Replicates results from lm2_PriceAd !!!
      results$lm2_AdjRSq_PriceAd2                                 <- round(summary(lm2_PriceAd2)$adj.r.squared,      3) # !!! Replicates results from lm2_PriceAd !!!
      ...
      results$lm2_b_AvgDailyUnitSales_PriceElas2AdAdj             <- round(summary(lm2_PriceAd2)$coefficients[2, 1], 2) * (results$lm0_a_AvgWeeklyUnitPrice_NoAd/
                                                                                                                           results$lm2_a_AvgDailyUnitSales_AvgPrice_NoAd)
      ...
      # If avg. unit cost are less than max. weekly unit price, then go ahead with price optimization...
      #if (results$AvgWeeklyUnitCost < max(data2$AvgWeeklyUnitPrice OR )) {
      ...
      Optimum_lm1a_PriceNotAdAdj <- optimize(ProfitFunction_lm1a_PriceNotAdAdj,
                                             maximum = T,
                                             lower   =                                 0,#results$AvgWeeklyUnitCost,
                                             upper   = max(data2$AvgWeeklyUnitPrice) + 2*results$StdWeeklyUnitPriceDev
                                             )
      ...
      ProfitFunction_lm2_PriceAdAdj_Ad          <- function(AvgWeeklyUnitPrice) {
            EstimatedDailyUnitSales <- predict(lm2_PriceAd, newdata = data.frame(AvgWeeklyUnitPrice = AvgWeeklyUnitPrice, AdFeature = "SALE"))
            EstimatedDailyUnitSales*
                  (AvgWeeklyUnitPrice-results$AvgWeeklyUnitCost)
      }
      
      # Maximize estimated daily profits...
      Optimum_lm2_PriceAdAdj_NoAd   <- optimize(ProfitFunction_lm2_PriceAdAdj_NoAd,
                                                maximum = T,
                                                lower   =                                 0,#results$AvgWeeklyUnitCost,
                                                upper   = max(data2$AvgWeeklyUnitPrice) + 2*results$StdWeeklyUnitPriceDev
                                                )
      Optimum_lm2_PriceAdAdj_Ad     <- optimize(ProfitFunction_lm2_PriceAdAdj_Ad,
                                                maximum = T,
                                                lower   =                                 0,#results$AvgWeeklyUnitCost,
                                                upper   = max(data2$AvgWeeklyUnitPrice) + 2*results$StdWeeklyUnitPriceDev
                                                )
      ...
      results$OptPrice_lm2_PriceAdAdj_Ad        <- Optimum_lm2_PriceAdAdj_Ad$maximum
      results$MaxProfit_lm2_PriceAdAdj_Ad       <- Optimum_lm2_PriceAdAdj_Ad$objective
      ...
      results$OptPrice_lm2_PriceAdAdj_Ad        <- NA
      results$MaxProfit_lm2_PriceAdAdj_Ad       <- NA
            
      }
      
      # If avg. unit cost are GREATER than max. weekly unit price, price optimization doesn't make sense and results are NA
      #} else {
            
      #results$OptPrice_lm1a_PriceNotAdAdj       <- NA
      #results$MaxProfit_lm1a_PriceNotAdAdj      <- NA
      ...
      #                         limits = c(            min(data2$AvgWeeklyUnitPrice, na.rm = T),             max(data2$AvgWeeklyUnitPrice, na.rm = T) + 2*results$StdWeeklyUnitPriceDev)) +
      #      # Add unit cost, optimal prices & maximezed profits
      #      geom_vline(xintercept = results$AvgWeeklyUnitCost,       size = .5, alpha = .3, linetype = "dotdash", color = "black") +
      #      geom_vline(xintercept = results$OptPrice_lm1a_PriceNotAdAdj,   size = .5, alpha = 1, linetype = "dotted", color = "springgreen1") +
      #      geom_vline(xintercept = results$OptPrice_lm2_PriceAdAdj_NoAd,  size = .5, alpha = 1, linetype = "dotted", color = "springgreen4") +
      #      geom_hline(yintercept = results$MaxProfit_lm1a_PriceNotAdAdj,  size = .5, alpha = 1, linetype = "dashed", color = "springgreen1") + 
      #      geom_hline(yintercept = results$MaxProfit_lm2_PriceAdAdj_NoAd, size = .5, alpha = 1, linetype = "dashed", color = "springgreen4") +
      #      geom_hline(yintercept = 0, size = .5, alpha = 1)
      ...
      #ggplot(data = data2,
            ...
            #scale_y_continuous(breaks = seq(    floor(min(data2$AvgDailyUnitSales,  na.rm = T)),   ceiling(max(data2$AvgDailyUnitSales,  na.rm = T)),
            #                                abs(floor(min(data2$AvgDailyUnitSales,  na.rm = T))-   ceiling(max(data2$AvgDailyUnitSales,  na.rm = T)))/10),
            #                   limits = c(      floor(min(data2$AvgDailyUnitSales,  na.rm = T)),   ceiling(max(data2$AvgDailyUnitSales,  na.rm = T)))) +
            #scale_x_continuous(breaks = seq(    round(min(data2$AvgWeeklyUnitPrice, na.rm = T), 0)-1, round(max(data2$AvgWeeklyUnitPrice, na.rm = T) + 2*results$StdWeeklyUnitPriceDev, 0)+1, 0.1),
            #                   limits = c(            min(data2$AvgWeeklyUnitPrice, na.rm = T),             max(data2$AvgWeeklyUnitPrice, na.rm = T) + 2*results$StdWeeklyUnitPriceDev))
      ...
      print(paste("UPC", SelectedProduct, "at Store", SelectedStore, "completed"))
}

results     <- ldply(ProductList, data.frame) %>%
      left_join(select(data1, -c(LikeCode,
                                 LikeCode_UPC,
                                 Product,
                                 Store,
                                 AvgWeeklyUnitPrice,
      ...
      select(Store,
             ...
             ## Model foundation
             # How often was item featured in ad?
             TotalAdFeat_abs,
             TotalAdFeat_rel,
             # Are cost accurate? Does profit function have to be repeated?
                   AvgWeeklyUnitCost,
             # How do prices change/deviate? Regardless of ad-feature and with regard to ad-feature condition
                   AvgWeeklyUnitPrice,
                   AvgWeeklyUnitPrice_Sig,
                   StdWeeklyUnitPriceDev,
                   AvgWeeklyUnitPriceChange,
             lm0_a_AvgWeeklyUnitPrice_NoAd,
             lm0_a_AvgWeeklyUnitPrice_NoAd_Sig,
                   StdWeeklyUnitPriceDev_NoAd,
                   AvgWeeklyUnitPriceChange_NoAd,
             lm0_b_AvgWeeklyUnitPrice_Ad,
             lm0_b_AvgWeeklyUnitPrice_Ad_Sig,
                   StdWeeklyUnitPriceDev_Ad,
                   AvgWeeklyUnitPriceChange_Ad,
             # How do price & ad-feature correlate?
             lm0_RSq_PriceAdCorr,
             lm0_AdjRSq_PriceAdCorrAdj,
             
             ## Compare model and choose best model
             # Test nested models with ANOVA p-values whether new variables are necessary or not
             ModelCompPrice,
             ModelCompPriceAd,
             ModelCompAd,
             ModelCompAdPrice,
             ...
             ## Determine optimal price & max. profit based on best model
             OptPrice_lm1a_PriceNotAdAdj,
             MaxProfit_lm1a_PriceNotAdAdj,
             OptPrice_lm2_PriceAdAdj_NoAd,
             MaxProfit_lm2_PriceAdAdj_NoAd,
             OptPrice_lm2_PriceAdAdj_Ad,
             MaxProfit_lm2_PriceAdAdj_Ad,
             
             ## Show parameters of model in detail
             
             # Outcome in units sold on avg. day per week...
             
             # Regardless of price nor ad feature
                    #AvgDailyUnitSales,
                    #AvgDailyUnitSales_Sig,
             
             # With regard to Price (Unadjusted for ad-feature)
             lm1a_a_AvgDailyUnitSales_ZeroPrice,
             lm1a_a_AvgDailyUnitSales_ZeroPrice_Sig,
             lm1a_a_AvgDailyUnitSales_AvgPrice,
             lm1a_a_AvgDailyUnitSales_AvgPrice_Sig,
             
             lm1a_b_AvgDailyUnitSales_PriceElasNotAdAdj,
             lm1a_b_AvgDailyUnitSales_PriceElasNotAdAdj_Sig,
             lm1a_b_AvgDailyUnitSales_PriceElas2NotAdAdj,
             lm1a_b_AvgDailyUnitSales_PriceElas2NotAdAdj_Sig,
             
             # With regard to Ad Feature (Unadjusted for price)
             lm1b_a_AvgDailyUnitSales_NoAdNotPrAdj,
             lm1b_a_AvgDailyUnitSales_NoAdNotPrAdj_Sig,
             lm1b_b_AvgDailyUnitSales_AdNotPrAdj,
             lm1b_b_AvgDailyUnitSales_AdLiftNotPrAdj,
             lm1b_b_AvgDailyUnitSales_AdNotPrAdj_Sig,
             
             # With regards to Price & Ad-Feature (Adjusted)
             lm2_a_AvgDailyUnitSales_ZeroPrice_NoAd,
             lm2_a_AvgDailyUnitSales_ZeroPrice_NoAd_Sig,
             lm2_a_AvgDailyUnitSales_AvgPrice_NoAd,
             lm2_a_AvgDailyUnitSales_AvgPrice_NoAd_Sig,
             
             lm2_b_AvgDailyUnitSales_PriceElasAdAdj,
             lm2_b_AvgDailyUnitSales_PriceElasAdAdj_Sig,
             lm2_b_AvgDailyUnitSales_PriceElas2AdAdj,
             lm2_b_AvgDailyUnitSales_PriceElas2AdAdj_Sig,
             
             lm2_c_AvgDailyUnitSales_ZeroPrice_Ad,
             lm2_c_AvgDailyUnitSales_ZeroPrice_Ad_Sig,
             lm2_c_AvgDailyUnitSales_AvgPrice_Ad,
             lm2_c_AvgDailyUnitSales_ZeroPrice_AdLiftPrAdj,
             lm2_c_AvgDailyUnitSales_AvgPrice_AdLiftPrAdj,
             lm2_c_AvgDailyUnitSales_AvgPrice_Ad_Sig
             )
...
filter(data0, SubDept %in% SelectedSDept & (WeekNo  %in% SelectedWeeks)) %>%
      write.csv(paste0(paste(SelectedSDept, collapse = " "), " Product Sales Data_", SelectedWeeks_Begin, " - ", SelectedWeeks_End, " (", SelectedWeeks_TotalAdWeeks,  " Ad Weeks).csv"), row.names = F)
