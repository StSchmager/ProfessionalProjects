...
Categories     <- openxlsx::read.xlsx(list.files(), colNames = F) %>% 
      select(X1:X3) %>% 
      rename(UPC        = X1,
             CategoryNo = X2,
             Category   = X3) %>% 
      ...
      mutate(Category   = as.character(Category),
             CategoryNo = as.character(CategoryNo),
...
Items$WeekAfter_factor   <- sapply(Items$WeekAfterNo_factor, function(x) {c(levels(Items$Week_factor)[2:levels(Items$Week_factor)], NA)[x]})
...
write.csv(DeptCatProd, "Category List_Depts & Items.csv", row.names = F)
